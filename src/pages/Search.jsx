...
export default function Search() {
  ...
  async function handleSearch(q) {
    ...
    try {
      ...
      setError('Search failed. Make sure proxy is running: cd ~/afaq/server && node proxy.js');
    }
    setLoading(false);
  }
    ...
    <div className="app">
      <div style={{ padding:'3rem 0 2rem' }}>
        ...
        <p style={{ fontSize:15, color:'var(--t3)', marginBottom:'2.5rem', fontWeight:300, lineHeight:1.6 }}>
          ...
          Grounded in verified canonical text from quran.com.
        </p>
        ...
        <div style={{ display:'flex', gap:10, marginBottom:'1.25rem', position:'relative' }}>
          ...
          <input
            ...
            placeholder="expanding universe, soul, embryo, iron..."
            style={{ flex:1, padding:'1rem 1.2rem 1rem 3rem', background:'var(--bg2)', border:'1px solid var(--b)', borderRadius:'50px', color:'var(--t1)', fontSize:15, fontFamily:"'DM Sans',sans-serif", outline:'none', transition:'border-color 0.2s, box-shadow 0.2s' }}
        ...
        {results && results.length > 0 && (
          <div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              {results.map((r, i) => (
                <div key={i}
                  style={{ background:'var(--card)', border:'1px solid var(--b)', borderRadius:'var(--r)', overflow:'hidden', position:'relative', transition:'all 0.25s' }}
                  onMouseOver={e => { e.currentTarget.style.borderColor='var(--b2)'; e.currentTarget.style.transform='translateY(-2px)'; }}
                  onMouseOut={e => { e.currentTarget.style.borderColor='var(--b)'; e.currentTarget.style.transform='translateY(0)'; }}
                >
                  {/* Top shimmer */}
                  ...
                  <div style={{ padding:'1.4rem 1.5rem 1.2rem 1.8rem' }}>
                    {/* Meta */}
                    ...
                    {/* Arabic */}
                    <div style={{ fontFamily:"'Tajawal',serif", fontSize:'clamp(18px,3vw,23px)', fontWeight:500, direction:'rtl', textAlign:'right', color:'var(--t1)', lineHeight:1.9, marginBottom:'0.85rem' }}>
                      {r.arabic}
                    </div>

                    {/* Divider */}
                    <div style={{ height:'1px', background:'var(--b)', margin:'0 0 0.9rem' }} />

                    {/* Translation */}
                    <div style={{ fontSize:14, color:'var(--t2)', lineHeight:1.75, fontStyle:'italic', fontWeight:300 }}>
                      {r.translation}
                    </div>

                    {/* Edition */}
                    {r.edition && (
                      <div style={{ fontSize:11, color:'var(--t3)', marginTop:'0.6rem' }}>
                        — {r.edition}
                      </div>
                    )}
                  </div>
            ...
            {/* MCP Attribution */}
            <div style={{ marginTop:'2rem', padding:'1rem 1.25rem', background:'var(--bg2)', borderRadius:'var(--r2)', border:'1px solid var(--b)', display:'flex', alignItems:'center', gap:10, fontSize:12, color:'var(--t3)', fontWeight:300 }}>
              <div style={{ width:6, height:6, borderRadius:'50%', background:'var(--c)', boxShadow:'0 0 6px var(--c)', flexShrink:0 }} />
              Powered by quran.ai MCP · search_quran tool · Verified Quran corpus from quran.com
            </div>
          </div>
        )}
      </div>
